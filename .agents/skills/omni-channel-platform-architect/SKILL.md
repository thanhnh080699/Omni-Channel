...
- Map channel connection statuses to UI indicator colors exactly:
  - **Green (Xanh lá)**: Connected (`connected`).
  - **Yellow (Vàng)**: Connecting, waiting for QR, or waiting for configuration (`connecting`, `waiting_qr`, `waiting_config`).
  - **Red (Đỏ)**: Error (`error`).
  - **Gray (Xám)**: Not connected / Disconnected (`not_connected`, `disconnected`).
  - *Note*: Ensure Error (Red) and Not Connected (Gray) are clearly distinguished rather than merged into a single indicator style.
