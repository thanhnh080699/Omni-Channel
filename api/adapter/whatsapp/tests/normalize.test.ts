...
test("ignores non-text Baileys events", () => {
  const normalized = normalizeBaileysMessage("ca_1", {
    key: { id: "wamid_empty", remoteJid: "84900000000@s.whatsapp.net", fromMe: false },
    messageTimestamp: 1780717200,
...
test("normalizes Baileys LID message with Alt JID", () => {
  const normalized = normalizeBaileysMessage("ca_1", {
    key: { 
      id: "wamid_lid_1", 
      ...
      participantAlt: "84923861999@s.whatsapp.net",
      fromMe: false 
    },
    messageTimestamp: 1780717200,
    pushName: "Huy Thành",
  ...
  assert.equal(normalized?.channel, "whatsapp");
  assert.equal(normalized?.external_message_id, "wamid_lid_1");
  assert.equal(normalized?.external_conversation_id, "84923861999@s.whatsapp.net");
  assert.equal(normalized?.sender_external_id, "84923861999@s.whatsapp.net");
  assert.equal(normalized?.text, "hello via lid");
});
